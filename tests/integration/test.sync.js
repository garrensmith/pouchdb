...
adapters.forEach(function (adapters) {
  describe("test.sync.js-" + adapters[0] + "-" + adapters[1], function () {
      ...
      function syncThenCancel() {
        promise = promise.then(function () {
          return new testUtils.Promise(function (resolve, reject) {
            ...
            return testUtils.Promise.all([db.destroy(), remote.destroy()]);
          });
        });
      }
    ...
    it('#3270 triggers "denied" events, reverse direction', function (done) {
      testUtils.isCouchDB(function (isCouchDB) {
        if (/*adapters[1] !== 'http' || */ !isCouchDB) {
          return done();
        }
      ...
      return remote
        ...
        .then(function () {
          return new testUtils.Promise(function (resolve, reject) {
            ...
            var sync = db
              .sync(remote, { filter: filter, live: true, retry: true })
              .on("error", reject)
              .on("change", onChange)
              .on("complete", resolve);
          });
