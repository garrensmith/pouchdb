...
adapterPairs.forEach(function (adapters) {
  describe.skip(
    "test.issue2674.js- " + adapters[0] + "-" + adapters[1],
    function () {
      var dbs = {};
      ...
      beforeEach(function () {
        ...
        dbs.fourthDB = testUtils.adapterUrl(
          adapters[1],
          "test_slash_ids_remote"
        );
      });

      afterEach(function (done) {
        testUtils.cleanup(
          [dbs.name, dbs.secondDB, dbs.thirdDB, dbs.fourthDB],
          done
        );
      });
      ...
      it("Should correctly synchronize attachments (#2674)", function () {
        ...
        function createDoc() {
          return dbA.put(doc).then(addRev).catch(handleError);
        }

        // add image1.jpg
        function addImg1() {
          return dbA
            .putAttachment(doc._id, "image1.png", doc._rev, img1, "image/png")
            .then(addRev)
            .catch(handleError);
        }
        ...
        function addImg2() {
          return dbA
            .putAttachment(doc._id, "image2.png", doc._rev, img2, "image/png")
            .then(addRev)
            .catch(handleError);
        }
        ...
        function updateDoc() {
          ...
          return dbB.put(newDoc).catch(handleError);
        }
        ...
        return (
          createDoc() // create document, no atts
